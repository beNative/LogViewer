...
export const Dropzone: React.FC<DropzoneProps> = ({ onFileDrop, error, iconSet, title, subtitle, details }) => {
  ...
  const handleDrop = React.useCallback((e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      logToConsole(`Dropzone: Received ${e.dataTransfer.files.length} files via drop.`, 'INFO');
      Array.from(e.dataTransfer.files).forEach(f => {
        logToConsole(`Dropzone: Dropped file - ${f.name} (${f.size} bytes, type: ${f.type})`, 'DEBUG');
      });
      onFileDrop(e.dataTransfer.files);
    } else {
      logToConsole('Dropzone: Drop event received but no files found.', 'WARNING');
    }
  }, [onFileDrop, logToConsole]);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      logToConsole(`Dropzone: Selected ${e.target.files.length} files via dialog.`, 'INFO');
