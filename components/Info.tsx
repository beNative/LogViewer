...
const parseMarkdown = (markdown: string): string => {
    ...
    // Tables - parse before other inline elements
    html = html.replace(/^(\|.+\|)\n(\|[-:| ]+\|)\n((?:\|.+\|\n?)+)/gm, (match, headerRow, separatorRow, bodyRows) => {
        ...
        const headerHtml = headers.map((h: string) =>
            `<th class="px-4 py-2 text-left font-semibold text-gray-700 dark:text-gray-300 border-b border-gray-300 dark:border-gray-600">${h}</th>`
        ).join('');

        // Parse body rows
        const rows = bodyRows.trim().split('\n').map((row: string) => {
            const cells = row.split('|').slice(1, -1).map((c: string) => c.trim());
            return `<tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50">${cells.map((c: string) =>
                `<td class="px-4 py-2 border-b border-gray-200 dark:border-gray-700">${c}</td>`
            ).join('')}</tr>`;
        }).join('');

        return `<div class="overflow-x-auto my-4"><table class="min-w-full border-collapse text-sm"><thead class="bg-gray-100 dark:bg-gray-800"><tr>${headerHtml}</tr></thead><tbody>${rows}</tbody></table></div>`;
    });
    ...
    // Lists (multi-pass, inside-out logic)
    for (let i = 4; i >= 0; i--) {
    ...
    html = html.replace(/<p>(\s*)<(ul|ol|li|blockquote|hr|pre|h[1-6]|div|table)/g, '<$2');
    html = html.replace(/<\/(ul|ol|li|blockquote|hr|pre|h[1-6]|div|table)>(\s*)<\/p>/g, '</$1>');

    // Cleanup <br> tags before lists inside other elements
                ...
                <nav className="flex-1 p-2 space-y-1">
                    {MARKDOWN_FILES.map((file, index) => (
                        <button
                            ...
                            className={`w-full text-left px-3 py-2.5 text-sm font-medium rounded-md transition-colors duration-150 ${activeTabIndex === index
